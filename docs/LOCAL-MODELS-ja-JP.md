...
## 8. llama.cppのオプションを理解する

以下は、`llama-server.exe`でよく調整する代表的なオプションです。まずは安定した基準値から始めて、出力が乱れすぎる、繰り返しが多い、遅すぎる、VRAMに収まりにくい、といった点に応じて調整してください。

| オプション | 何を制御するか | 実用上の目安 |
| --- | --- | --- |
| `--temp` | 出力のランダム性 | 値を下げるほど決定的になります。`0`は完全に決定的で、`0.7`〜`0.8`前後は会話用途でよく使われます。 |
| `--top-p` | nucleus samplingのしきい値 | 値を下げるほど保守的になります。`1.0`で無効、`0.9`〜`0.95`前後がよく使われます。 |
| `--top-k` | 候補トークン数の上限 | 値を下げるほど出力の幅が狭くなり、より焦点が合いやすくなります。`0`で無効です。 |
| `--presence-penalty` | すでに登場したトークンへのペナルティ | 同じ話題や表現の反復を減らしたいときに使います。不要なら`0`のままで構いません。 |
| `--repeat-penalty` | 直近で繰り返されたトークンへのペナルティ | `1.05`のように`1.0`より少し大きくすると、ループや言い回しの反復を抑えやすくなります。 |
| `--ctx-size` | コンテキストウィンドウのサイズ | 大きいほど会話履歴やファイルを多く読めますが、その分VRAMやRAMを使います。 |
| `--n-gpu-layers` | GPUへオフロードする層数 | `all`、`auto`、数字指定などを使います。VRAMに余裕があるなら増やします。 |
| `--n-cpu-moe` | CPU側に残すMixture-of-Experts層数 | MoEモデルでのみ意味があります。モデルやビルドの都合でCPUに残したい場合に調整します。 |
| `--jinja` / `--no-jinja` | Jinjaのチャットテンプレートを使うかどうか | ほとんどのチャットモデルでは`--jinja`のままでよいです。独自のプロンプト形式が必要なモデルだけ切り替えます。 |
| `--cache-type-k` | KVキャッシュのデータ型 | 低精度のキャッシュほどメモリ使用量が下がります。`f16`が無難で、`q4_0`や`q4_1`は節約向きです。 |
| `--sleep-idle-seconds` | しばらく使われなかったときにモデルを自動解放するまでの秒数 | `-1`で無効化します。待機時にメモリを空けたい場合は正の値を指定します。 |

これらのオプションは、すべてのモデルで必須ではありません。課題に合うものだけを調整してください。

## 9. llama-server.exeを起動する
...
## 11. Claw Codeからllama-server.exeへ接続する
